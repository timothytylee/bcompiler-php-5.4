...
/* $Id: bcompiler_zend.c 316140 2011-09-05 10:16:36Z val $ */
...
#endif
#ifdef ZEND_ENGINE_2_4
			if (zo->op1.jmp_addr >= zoa->opcodes)
				zo->op1.opline_num = zo->op1.jmp_addr - zoa->opcodes;
#else
			if (zo->op1.u.jmp_addr >= zoa->opcodes)
...
#endif
#ifdef ZEND_ENGINE_2_4
			if (zo->op2.jmp_addr >= zoa->opcodes)
				zo->op2.opline_num = zo->op2.jmp_addr - zoa->opcodes;
#else
			if (zo->op2.u.jmp_addr >= zoa->opcodes)
